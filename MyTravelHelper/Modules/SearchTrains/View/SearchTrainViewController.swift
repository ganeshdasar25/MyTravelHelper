...
extension SearchTrainViewController:UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        dropDown.dataSource = stationsList.map {$0.stationDesc}.sorted()
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if let inputedText = textField.text {
            ...
            dropDown.dataSource = stationsList.filter{$0.stationDesc.lowercased().hasPrefix(desiredSearchText.lowercased())}.map{$0.stationDesc}.sorted()
            dropDown.show()
...
extension SearchTrainViewController:UITableViewDataSource,UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: TrainInfoCell.reuseIdentifer, for: indexPath) as? TrainInfoCell
        ...
        if let _destinationDetails = train.destinationDetails {
            cell.destinationInfoLabel.text = _destinationDetails.locationFullName ?? "Unknown"
